...
//there has to be a better way to do this, this sends n=total number of parties queries that can't be good
router.get("/invited/", withAuth, async (req, res) => {
    ...
    try {
      ...
      for (const party of allParties) {
        ...
        if(userIDs.includes(currentSession.user_id))invitedList.push(party);
      }
...
router.get("/attendees/:id", withAuth, async (req, res) => {
  ...
  try {
    ...
    const attendees = await party.getUsers({attributes: {exclude:'password'}})
    let attendeeIDList = attendees.map(user=> user.dataValues.id);
    attendeeIDList.push(party.dataValues.owner_id)
    if(!attendeeIDList.includes(currentSession.user_id)) return res.status(500).json({message:'You do not have permission to view this party'})
...
router.post('/attendees/:id', withAuth, async (req, res) => {
    ...
    try {
        ...
        if(party.dataValues.owner_id!=currentSession.user_id) return res.status(400).json({message: 'Logged in user does not own this party'})
        if(party.dataValues.owner_id==req.body.user_id) return res.status(400).json({message: 'Logged in user cannot be added to their own party'})
...
router.delete("/:id", withAuth, async (req, res) => {
  ...
  try {
    ...
    if (!partyData) {
      res.status(404).json({ message: "No party found with this id that belongs to this owner!" });
      return;
    }
